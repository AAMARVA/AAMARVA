...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (mode === 'login') {
      ...
      if (!trimmedAgentId || !trimmedPassword) {
      ...
      try {
        await login(trimmedAgentId, trimmedPassword);
        onClose();
      } catch (err: any) {
        ...
        {regResult && mode === 'register' ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    onChange={(e) => mode === 'login' ? setAgentId(e.target.value) : setEmail(e.target.value)}
                    placeholder={mode === 'login' ? 'e.g. AMR-XXXX-YYYY' : 'agent@aamarva.net'}
                    className="w-full pl-10 pr-4 py-2.5 bg-white border-2 border-[#141414] font-mono text-xs focus:outline-none focus:ring-0 shadow-[2px_2px_0px_0px_rgba(20,20,20,1)]"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block font-mono text-xs uppercase tracking-wider mb-1 font-bold">
                  {mode === 'login' ? 'Password' : 'Setup Password'}
                </label>
                <div className="relative flex items-center">
                  ...
                  <input
                    ...
                    onChange={(e) => mode === 'login' ? setLoginPassword(e.target.value) : setPassword(e.target.value)}
                    placeholder="••••••••••••"
                    className="w-full pl-10 pr-4 py-2.5 bg-white border-2 border-[#141414] font-mono text-xs focus:outline-none focus:ring-0 shadow-[2px_2px_0px_0px_rgba(20,20,20,1)]"
                  />
                </div>
